...
from airpay.models import AirPlan, AirSeller, Subscriptions
...
from airpay.serializers import (
    AirPlanSerializer,
    RazorpayRouteOnboardingDetailsSerializer,
...
class CreateSubscriptions(ListAPIView):
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            if (
                subscription.status == "pending"
                and not subscription.order_id
                and not subscription.subscription_id
            ):
        ...
        content=json.dumps(
            {
                ...
                "BUSINESS_CATEGORY": [
                    {"label": y, "value": x} for (x, y) in BUSINESS_CATEGORY
                ],
                "SUB_BUSINESS_CATEGORY": [
                    {"label": x.replace("_", " ").capitalize(), "value": x}
                    for x in BUSINESS_SUB_CATEGORY[business_category]
                ]
                if business_category is not None
                else [],
                "REQUIRED_FIELDS": [
                    doc
                    for doc, data in KYC_DOCUMENTS.items()
                    if business_type in data["business_types"]
                ]
